...
# Exécute les six preuves fonctionnelles de la démo APISIX déjà démarrée.
# Chaque assertion échoue avec un code non nul afin d'être réutilisable en CI.

set -Eeuo pipefail

# Les chemins sont ancrés sur le service, indépendamment du dossier de lancement.
...
[[ -f "$ENV_FILE" ]] || fail "${ENV_FILE} absent ; lancez d'abord ./scripts/start-apisix-demo.sh depuis la racine du service"
...
printf '\n1/6 - Requête partenaire sans clé : APISIX doit refuser\n'
"${compose[@]}" run --rm --no-deps partner-client --without-key --expected-status 401 >"$unauthorized_response"
printf 'OK : HTTP 401\n'

printf '\n2/6 - Partenaire -> APISIX -> API interne fictive\n'
...
printf '\n3/6 - SI interne -> APISIX -> partenaire HTTPS/mTLS\n'
...
printf '\n4/6 - Route OIDC sans bearer token : APISIX doit refuser\n'
"${compose[@]}" run --rm --no-deps oidc-partner-client --without-auth --expected-status 401 >"$unauthorized_response"
printf 'OK : HTTP 401 sans access token\n'

printf '\n5/6 - Partenaire -> Keycloak -> APISIX OIDC -> API interne\n'
oidc_response="$("${compose[@]}" run --rm --no-deps oidc-partner-client)"
jq -e '.service == "api-interne-commandes" and .request.oidc_access_token == true' \
  <<<"$oidc_response" >/dev/null
printf 'OK : client_credentials, bearer token et validation JWKS\n'

printf '\n6/6 - APISIX -> Logstash -> Elasticsearch\n'
count=0
...
(( count >= 3 )) || fail "moins de trois journaux APISIX trouvés dans Elasticsearch"

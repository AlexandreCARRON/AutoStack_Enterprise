...
usage() {
  cat <<'EOF'
Usage: ./scripts/generate-docker-compose.sh service1[/variante] [service2[/variante] ...]
...
Exemple :
  ...
  ./scripts/generate-docker-compose.sh Odoo/Odoo_v17 Z_Nextcloud/Nextcloud_v33.0.6
...
for requested_service in "$@"; do
  ...
  # Chaque fragment peut aussi fonctionner depuis son dossier de service copié.
  # Le générateur fixe ici sa racine lorsqu'il assemble les fragments au dépôt complet.
  service_token="$(printf '%s' "$service_name" | tr '[:lower:]' '[:upper:]' | tr '.-' '__')"
  service_dir_var="AUTOSTACK_${service_token}_DIR"
  printf '%s=./services/%s\n' "$service_dir_var" "$service_name" >> "$env_tmp"
  printf '%s\n' "$service_dir_var" >> "$required_vars"
done
